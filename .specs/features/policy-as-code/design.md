...
- **Interfaces** — todas `check(text: str, path: str, root: Path) -> list[PolicyViolation]` (assinatura única; `root` usado só para contextos de caminho):
  ...
  - `check_pathlib(text, path, root)` — **REQ-POL-08** (escopo **estreitado por emenda aprovada IMP-03**): flagga apenas sinais inequívocos de caminho bruto — (a) primeiro argumento posicional do builtin `open(...)`, (b) prefixo de drive Windows (`^[A-Za-z]:[\\/]`) ou (c) valor de atribuição a variável de nome path-named não-privada (`path/file/dir/output`) contendo separador ou extensão conhecida (`.pdf .json .toml .md .txt .csv .log .py .lock .env .example`, constante `_PATH_EXTENSIONS`). NÃO flagga strings que apenas terminam em extensão ou contêm `/` em outros contextos (fixtures `source="bula.pdf"`, `"5/minute"`, `"BAAI/bge-reranker-base"`, `"text/event-stream"`). Exceções: strings dentro de `Path(...)`/`str(...)`, docstrings, URL (`://`) e placeholder de formatação são ignoradas; padrões regex (argumentos de `re.*`) também.
  ...
  - `check_logger(text, path, root)` — **REQ-POL-10** (escopo **confirmado por emenda aprovada IMP-02**): módulos sob raiz `src/` com conteúdo executável precisam de atribuição módulo-nível `logger = logging.getLogger(__name__)` (AST `Assign` com `Name logger` e call `logging.getLogger`); **todo** `__init__.py` é isento (mesmo não-vazio — são marcadores de pacote/pontos de re-export, ex.: `src/medasist/evaluation/__init__.py`).
  - `check_docstring(text, path, root)` — **REQ-POL-11/20** (escopo **estreitado por emenda aprovada IMP-01**): vale apenas para arquivos de `src/`. Símbolos **públicos** (top-level ou métodos, nome sem `_` inicial e não-dunder) precisam de docstring (primeiro statement do corpo é string `Expr`); privados/dunders/vazios isentos. Arquivos de `tests/` e `scripts/` estão fora da regra (funções de teste sem docstring e arquivos de teste >800 linhas são a norma no repo). Validação de **presença**; estilo NumPy completo (seções) fica como melhoria futura documentada.
  - `check_complexity(text, path, root)` — **REQ-POL-12/19** (escopo **estreitado por emenda aprovada IMP-01**): vale apenas para arquivos de `src/` (limites físicos são convenção de código-fonte; arquivos de teste podem excedê-los sem baseline). Três sub-checks no mesmo módulo:
...
**Nota:** as entradas são **geradas** pela task de baseline population (T12) a partir da árvore real. O exemplo acima ilustra o formato; o baseline real verificado contra o worktree tem **29 entradas**: **23 FUNC-LENGTH** (as 7 AD-005 originais — `chain.py: _run_single/_merge_sub_results/_stream_single/stream_answer`, `retriever.py: retrieve/_retrieve_hybrid`, `pipeline.py: ingest_document` — mais 16 funções >50 linhas não mapeadas na pesquisa: `logging_setup.configure_logging`, `api/main.create_app`, `evaluation/metrics._collect_rows/_build_per_question`, `chain.run_query`, `ingestion/chunker.chunk_document`, `pipeline.ingest_directory`, `profiles/get_profile_config`, `retriever._rrf_fuse/_log_retrieve_metric`, `ui/app._render_streaming/main`, `ui/client.query/query_stream`, `api/routers/ingest.ingest`, `api/routers/query._stream_events`), **2 NESTING-DEPTH** (`ui/app._delta_generator`, `ui/app.main`), **3 LOGGER** (`api/deps.py`, `api/schemas.py`, `ingestion/schemas.py` — módulos `src/` sem logger) e **1 NO-PRINT** (`scripts/evaluate_rag.py:_print_report`). Nenhum baseline para: future-import após docstring (~10 arquivos, legais por OQ-02), `__init__.py` vazios (~17, isentos por OQ-02), helpers privados sem docstring (3, isentos — regra só para públicos), pathlib/dados/800-linhas (limpos na árvore). O desvio do plano original ("8 entradas") foi aprovado como emenda (IMP-01/02/03 — ver `spec.md` "Emendas aprovadas").
