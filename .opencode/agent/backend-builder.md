...
4. **AGENTS.md** — project rules, conventions, and architecture constraints

Read the spec file(s) and AGENTS.md before writing a single line. The on-disk spec is the source of truth. If the spec and AGENTS.md conflict, surface the conflict — do not silently pick one.
...
1. **Write the test first** — red, then green. One failing test before any implementation code. Follow the project's testing conventions from AGENTS.md.
...
Keep business logic out of route handlers. Follow the layering pattern the researcher documented. Functions under 50 lines. No nesting beyond 4 levels (AGENTS.md rule).
...
### Unit tests
Every function you write gets a test. Follow this project's test naming convention from AGENTS.md:
